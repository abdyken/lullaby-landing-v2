...
onMounted(() => {
  ...
  // The card and phone now share one source of truth: the card whose centre is
  // nearest the reader's viewport line. IntersectionObserver batches boundary
  // crossings and only reports changed entries, which made the old screen state
  // briefly drift from the highlighted card during a fast or trackpad scroll.
  const syncActiveMoment = () => {
    animationFrame = 0
    ...
    const readerLine = window.innerHeight * 0.5
    const currentIndex = activeProductMomentIndex.value
    let nextIndex = currentIndex
    let closestDistance = Number.POSITIVE_INFINITY
    let currentDistance = Number.POSITIVE_INFINITY

    momentItems.forEach((item) => {
      const index = Number(item.dataset.momentIndex)
      if (!Number.isFinite(index)) {
        return
      }

      const bounds = item.getBoundingClientRect()
      const distance = Math.abs(bounds.top + bounds.height / 2 - readerLine)
    ...
    // Ask the next card to be meaningfully closer before changing state. This
    // removes the flicker at the midpoint of two cards while keeping the
    // phone and the highlighted card driven by the same value.
    // A generous deadband deliberately gives each story beat more breathing
    // room during ordinary wheel / trackpad scrolling. It applies in both
    // directions, so reversing direction feels just as calm as moving ahead.
    const switchThreshold = 120
    const shouldSwitch =
      nextIndex !== currentIndex &&
      closestDistance + switchThreshold < currentDistance
  ...
  const requestMomentSync = () => {
    if (!animationFrame) {
      animationFrame = window.requestAnimationFrame(syncActiveMoment)
    }
  }
  ...
  const removeScrollListeners = () => {
    ...
    isListening = false
    window.removeEventListener('scroll', requestMomentSync)
    window.removeEventListener('resize', requestMomentSync)
  }
  ...
  const addScrollListeners = () => {
    ...
    window.addEventListener('resize', requestMomentSync)
    requestMomentSync()
  }
          ...
          <div class="hero-shrink-stage">
            ...
            <div class="hero-inner">
              <div class="hero-copy">
                ...
                <a
                  class="app-store-badge app-store-badge--hero"
                  :href="APP_STORE_URL"
                  target="_blank"
                  rel="noopener noreferrer"
                ...
                >
                  ...
                  <span class="app-store-badge-copy">
                    <span class="app-store-badge-kicker">Download on the</span>
                    <span class="app-store-badge-title">App Store</span>
                  </span>
                </a>
              </div>
      ...
      <section id="download" class="final-cta section">
        <div class="container final-inner reveal-item">
          ...
          <div class="final-action">
            <a
              class="app-store-badge app-store-badge--final"
              :href="APP_STORE_URL"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Download Lullaby on the App Store"
